...
**まだ実装していない。** 案を捨てないために書き残す。

出発点は「バインダーを2つ作り、公開先URLを `/ja` `/en` で分ける」という話だった。
運用コストへの懸念から他の設計も検討したが、
**「サイト管理はBinderの命題ではない」という前提**（次節）に立ち返ると、
現時点の結論は**案E（クローンで作る2バインダー）**に戻る。
案Fはコアに言語の概念を持ち込むため、いまは採らない。

## 前提: サイト管理はBinderの命題ではない

**この制約が他のすべてに優先する。**

Binderはメモを取るためのツールであり、サイト管理はその一部の用途にすぎない。
したがって国際化の仕組みは、**メモを取るだけの人の目に触れてはいけない**。
極端な例として、メモを取るのが目的の人に
「このノートは en です」という入力をさせるようなことはしない。

当初「欲しいならバインダーを2つ作る」と決めていたのは、この理由による。
アプリ本体に何も足さずに済む、というのが最大の価値だった。

この制約で案を測ると評価が変わる。

| 案 | メモを取る人への影響 |
|---|---|
| 案A / 案E | **ゼロ**。アプリに何も足さない。運用パターンにすぎない |
| 案F | `langs` 未宣言なら画面上はゼロ。ただし**コア（パス解決・doctor・Status）に言語の概念が入る** |
| 案D | ツリーに言語が現れる。**論外** |

案Fの「未翻訳一覧」「陳腐化検出」「アセット共有」は確かに便利だが、
**それらはサイト管理の機能**であり、まさに命題から外れる部分である。
利便性と引き換えにコアを太らせる取引になっている。
...
## 案F: IDはそのまま、実体側にバージョンを持たせる（保留）

> **採らない。** 設計としては最も筋が良いが、コア（パス解決・doctor・Status）に
> 言語の概念が入るため「サイト管理はBinderの命題ではない」に反する。
> Binderがサイト管理を命題に含めると決めたときに再検討する。
...
**国際化されたアプリを説明するサイトでは、画面そのものが言語ごとに別物になる。**
日本語版のページには日本語UIのスクショ、英語版には英語UIのスクショが要る。
つまり `assets/{id}.en` に**別の画像を置くのが基本**であり、ここは省略できない。
画面説明サイトでは、これがアセットを最大の翻訳負債にする。

### レイヤーの位置づけ

レイヤーは**アセット（画像）に SVG の図形とテキストを重ねる**機能で、
注釈テキストと座標は `layers/{id}.json` 側にある（[layer.go](../layer.go) `LayerShape.Text` / `buildSVG`）。

- 注釈テキストの翻訳が要るのは当然として、**英語UIは文字幅もレイアウトも変わるので座標もずれる**。
  結局 `layers/{id}.en.json` が言語ごとに要る
- レイヤーが撮り直しを省けるのは、下地が**言語非依存の画像**（構成図、写真、
  言語が変わらない外部サービスの画面など）の場合だけ。UIスクショでは効かない
- ただし**参照の一貫性は効く**。レイヤーは親アセットをIDで参照するので、
  `resolve(assetId, lang)` が en 画像を返せば、レイヤー定義の側は
  「同じID」のまま言語版を差し替えられる。IDを付け替える作業は発生しない
...
- **スクショの撮り直し運用**をどう支援するか。言語ごとにアプリを起動して撮る必要があり、
  「どのアセットが未翻訳か」の一覧が実質的な作業リストになる
...
> 以下は**案Fを採る場合**の設計。現時点では保留。
> 案Eでは「対応言語」の入力自体が不要で、アプリは言語を知らないままになる。
...
```json
"langs": [
  { "code": "ja", "dir": "",   "name": "...", "detail": "...", "default": true },
  { "code": "en", "dir": "en", "name": "...", "detail": "..." }
]
```

- **宣言することが機能のスイッチになる。** 言語が宣言されて初めてアプリは
  「en版はどういう状態か」（未翻訳・陳腐化・未公開）を計算できる。
  `langs` が無いバインダーは今までと完全に同じ挙動＝**既存バインダーへの影響ゼロ**
- 言語別のサイト名・説明はここに持つ（`.Home.Name` / `.Home.Detail` の言語対応）
- **既定言語（`.{lang}` を付けない側）は後から変えられない**。変えると全実体ファイルの
  リネームになる。初回設定時のみ変更可とし、以降は警告を出す

### 編集中の言語は「アプリの状態」であって設定ではない

- 現在編集している言語の選択は `setting.json`（ホーム配下）にバインダー単位で保存する。
  `binder.json` に入れるとコミット対象になり、共同作業者の作業状態を上書きしてしまう
- 既定言語を選んでいる間は現在と全く同じ画面。翻訳言語を選ぶと、
  ツリー・エディタ・一覧がその言語の実体を見る

### ツリーに翻訳状態を出す

既存の Status（未記録・未公開）と同じ場所に、選択言語に対する状態を出せる。

| 状態 | 判定 |
|---|---|
| 未翻訳 | `notes/{id}.{lang}.md` が無い |
| 翻訳済み | 実体があり、原文より新しい |
| **原文が更新済み（陳腐化）** | 原文の最終コミット > 翻訳の最終コミット |

未翻訳ノートを開いたら「原文から複製して開始」、陳腐化なら
「前回翻訳時点からの原文の差分」を出す（翻訳ファイルの最終コミット時刻を基準に
原文の diff を取れば近似できる）。

**画面説明サイトでは、この一覧の主役はノートではなくアセットになる。**
未翻訳アセット一覧が、そのまま「撮り直すスクショの作業リスト」になる。

### 記録・公開・送信

- 未記録一覧はファイル単位なので、`{id}.en.md` もそのまま乗る。特別扱い不要
- 公開は言語ぶんループして `docs/` を作り、**送信は1回**
- 未公開一覧に言語の軸が増える

### 実装上の罠: doctor が翻訳ファイルを孤児と誤認する

`orphanContentIds()`（[merge_reconcile.go](../merge_reconcile.go)）は `notes/*.md` を舐めて
拡張子を落とした名前をIDとみなす。`notes/{id}.en.md` は **`{id}.en` という未知のID**に見えるため、
structure 行が無い孤児として**ツリーに新規ノードとして復元されてしまう**。

これは `RunDoctor` 経由で**バインダーを開くたびに走る**。案Fを入れるなら最初に塞ぐ。

## 閲覧者から見た言語切替

### URL

alias は言語共通なので、URLは機械的に対応する。

```
/pages/{alias}.html        既定言語
/en/pages/{alias}.html     en
```

### 切替の動作

**同じページの相手言語版へ移動する。** トップに飛ばさない。
alias が同じなのでリンクは生成できる。

問題は**相手言語に翻訳が無いページ**。3通りある。

| | 動作 | 評価 |
|---|---|---|
| (a) | 切替リンクを出さない | 正直だが、ページによって切替が消えて不親切 |
| (b) | 相手言語のトップへ飛ばす | よくあるが読者は迷子になる |
| (c) | **フォールバックして出す**（既定言語の本文を en 側にも出力） | URL構造が完全に対称になる |

**(c) を既定に、ページ上部へ「まだ翻訳されていません（原文表示）」のバナー**を推す。
理由は URL の対称性で、切替リンク・sitemap・内部リンクが単純になり、リンク切れが出ない。
バナー文言は辞書（`t`）に置く。生成時に (a) を選べるようにしておくとよい。

ただし SEO 上、フォールバックしたページは重複コンテンツになるので、
そのページには `hreflang` を出さない（または canonical を原文へ向ける）。

### 読者の選択を記憶するか

**しない方が良い。** 共有されたURLを開いた人が、意図しない言語へ飛ばされる。
切替は明示操作のみとする。`Accept-Language` による自動振り分けは、
静的ホスティング（GitHub Pages 等）では**そもそもできない**。

### テンプレートに要るもの

切替は静的な `<a href>` で足りるが、テンプレートから書けるように DTO へ渡す。

- `.Langs` — `code` / `name` / `url`（現在のページの相手言語版）/ `translated` の一覧。
  `{{ range .Langs }}` で切替UIが書ける
- `.Lang` — 現在の言語（`<html lang>` と `hreflang` に使う）
- `.Note.Alias`（既出）

アセットのURLも同じ解決に乗せる。翻訳版があれば `/en/assets/...`、
無ければ既定言語側を指す（未翻訳スクショを複製しない）。
...
| スクリーンショット | 二重 | バリアントが要る | 二重 | `assets/{id}.en`（言語ごとに撮り直しは不可避）|
| 注釈テキスト | 二重 | バリアントが要る | 二重 | `layers/{id}.en.json`（座標もずれるので言語別）|
...
「サイト管理はBinderの命題ではない」を優先するため、**案Eを採る**。

**案Eは案Aの否定ではなく、案Aの作り方を固定したものである。**
「バインダーを2つ作る」という当初の決めはそのままに、
2つ目を**手作りではなくクローンで作る**と決めるだけで、案Aの致命傷が消える。

| 案Aの問題 | クローンで作るとどうなるか |
|---|---|
| IDが別物になり `embed` / `assets` / リンクが壊れる | **IDが一致する。参照がそのまま動く** |
| テンプレートを移植できない | そのまま動く。マージで同期もできる |
| alias・ツリーを人力で揃える | `structures.csv` の3-wayマージで伝播する |
| 陳腐化を検出できない | 取り込み時のコンフリクトが検出そのものになる |

**アプリ本体には何も足さない。** 必要な改修はすべて公開・テンプレート側にあり、
メモを取るだけの人は一切触れない（「共通の小改修」節）。

捨てるものも明記しておく。未翻訳一覧・陳腐化の明示的な表示・アセットの共有は諦める。
ただし**画面説明サイトではスクショは言語ごとに撮り直しが要る**ため、
案Fの「アセット共有」の利点は実際にはほとんど効かない。
残る重複はテンプレート・CSS・言語非依存の図で、これらはマージで同期できる。

案Fは、**Binderがサイト管理を命題に含めると決めたとき**に再検討する。
...
- `binder.json` の `publishBranch` / `publishSubDir` の設定
- 「取り込み」で原文の更新を持ち込み、コンフリクトを翻訳作業として処理する流れ
- 実体ファイルは**IDが一致している前提**なので、
  翻訳作業は `notes/{id}.md` の中身を差し替えるだけであること
- 辞書アセット（`t`）の翻訳

ただし公開・送信の運用はメモ整理（`binder-organize`）とは目的が違うため、
同じスキルに入れるか別スキルにするかは要検討。
