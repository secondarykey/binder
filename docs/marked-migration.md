...
どちらの変更が原因か分からなくなる。v17（list トークン構造）と v18（末尾空行トリム）は
別種の破壊的変更であり、いずれも実測では同梱プラグインに影響しないが、
実測でカバーできない領域（ユーザ自作プラグイン・目視でしか分からない描画差）が残る。

なお「18 で困ったユーザが留まれる版を残す」は分ける理由にならない。`markedUrl`
