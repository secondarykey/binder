...
新しい移行を追加する具体的な手順は `setup/convert/AGENTS.md` を参照。
...
具体的な手順（`migrations` リストへの追加方法、`migrateState` の使い方等）は `setup/convert/AGENTS.md` の「移行の追加方法」節を参照。上記のパターン集はその際の実装リファレンスとして使う。
