...
func (a *App) SaveTemplate(id string, data string) error {
	...
	err := a.current.WriteTemplate(id, []byte(data))
	if err != nil {
...
func (a *App) CreateTemplateHTML(id string, elm string) (string, error) {

	if a.current == nil {
		return "", fmt.Errorf("Not Open Binder")
	}

	html, err := binder.CreateTemplateHTML(a.current, id, elm)
	if err != nil {
