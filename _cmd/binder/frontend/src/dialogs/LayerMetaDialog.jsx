...
import { EditLayer, GetLayer, RemoveLayer } from "../../bindings/binder/api/app";
 ...
 */
function LayerMetaDialog({ open, id, onClose }) {
  ...
  const handleSave = () => {
    if (!name) { showWarning(t("layer.nameRequired")); return; }
    if (!alias) { showWarning(t("layer.aliasRequired")); return; }

    EditLayer({ id, parentId, name, detail, alias, private: isPrivate, publish: publish || ZERO_TIME, republish: republish || ZERO_TIME }).then(() => {
      evt.markModified(id);
      evt.refreshTree();
      evt.showSuccessMessage(t("layer.updateSuccess"));
      onClose();
    }).catch((err) => {
  ...
  const handleDeleteConfirm = () => {
    ...
    RemoveLayer(id).then(() => {
      evt.refreshTree();
      evt.showSuccessMessage(t("layer.removeSuccess"));
      onClose();
      if (parentId) nav("/editor/assets/" + parentId);
    }).catch((err) => showError(err));
  };

  return (<>
    <MetaDialog
      open={open} onClose={onClose} title={t("layer.editTitle")}
        ...
        <AccordionDetails sx={{ px: 0, pt: 0, display: "flex", flexDirection: "column", gap: 1 }}>
          <FormControl>
            ...
            <TextField
              ...
              InputProps={{
                startAdornment: <InputAdornment position="start"><FormLabel>/layers/</FormLabel></InputAdornment>,
    ...
    <ConfirmDialog
      ...
      message={t("layer.deleteConfirm", { name })}
...
export default LayerMetaDialog;
